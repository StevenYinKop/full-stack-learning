...
var Timer = function () {
    ...
    _createClass(Timer, [{
        ...
        value: function countdown(end, update, handle) {
            ...
            var self = this; // 获取当前对象的指针
            if (now - end) {
                //如果当前时间大于截止时间, 说明倒计时已经结束了
                handle.call(this); // 执行倒计时结束的回调
            } else {
                var last_time = end - now; // 判断当前时间距离截止时间的剩余时间
                var px_d = 1000 * 60 * 60 * 24; // 一天的毫秒值
                var px_h = 1000 * 60 * 60; // 一小时的毫秒值
                var px_m = 1000 * 60; // 一分的毫秒值
                var px_s = 1000; // 一秒的毫秒值
                // 剩余时间包含多少天
